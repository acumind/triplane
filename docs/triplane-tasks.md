...
| D19 | **FIXED.** An approved proposal merged into `main` (PR #5, `bundles/meridian/guides/onboarding-user.md`) and then appeared nowhere: not on the site, not to the agent. Cause: **no Vercel project is connected to the repo** — all four report `link: null`, zero deploy hooks, and every deployment is `source=cli`. `setup-vercel.sh` ran `vercel git connect ... 2>&1 || echo "(git already connected)"`, which turned a real failure into a reassuring message. The underlying failure is Vercel's account-level GitHub authorisation having expired (`git-namespaces` → GitHub `401 Bad credentials`), which no `VERCEL_TOKEN` can repair — it needs the OAuth flow in the browser. Fixed here: the script now prints the failure, `docs/deploy.md` documents it, and `/govern` distinguishes its two very different approvals — the fs store rebuilds and says **Published**, the GitHub store merges and now says **merged into main, nothing here has changed yet**, with the unchanged bundle hash as the evidence. Previously `rebuilt: false` rendered no notice at all, so an approval looked like a publish that vanished. **Resolved 2026-09-03:** GitHub reauthorised in Vercel, all four projects linked to `acumind/triplane@main` via `POST /v9/projects/<name>/link`, and all four redeployed from commit `f11044e` — hosted meridian went 12 → 13 concepts (`864b8d1bfab2` → `00fb509fb3f2`) and now serves `onboarding-user`. Deployments are git-sourced from here on, so the next approval redeploys on its own. | `app/api/govern/route.ts`, `app/govern/page.tsx`, `scripts/setup-vercel.sh`, `docs/deploy.md` | T5, T7 |
| D18 | **FIXED** — Half the app had no door. `/sandbox`, `/concepts`, `llms.txt` and the raw OKF bundle were linked only from `Landing`, which renders on Triplane's own deployment and never on a tenant — so on meridian, controls and dhruva they were reachable only by typing the URL, and the demo script said so in as many words. The two demo switches (`?reviewer=1`, `?access=reader`) were query params with no control at all. `lib/destinations.ts` is now the one list of non-concept destinations, read by a new `ShellNav` block at the foot of the sidebar (index, review queue, sandbox, a **Machine planes** menu over the four plane-3 artifacts, and a **Viewing as …** menu holding both switches) and by ⌘K, which leads with them. `Menu` gained `drop="up"` because `.col-side` clips its overflow. Also removed `TopNav` and `Rail`, two unreferenced nav components, and dropped the hardcoded "Triplane" crumb on `/sandbox` — a client bundle cannot read the brand from config, so it said Triplane on every tenant. | `lib/destinations.ts`, `ShellNav.tsx`, `CommandPalette.tsx`, `ConceptSidebar.tsx`, `Menu.tsx`, `Icon.tsx`, `app/sandbox/page.tsx`, `docs/demo-script.md` | T7 |
| D17 | **FIXED** — "New concept" (and "Propose change") dropped a non-reviewer on `/govern` with no explanation of why they were there: the page said only that reviewer mode was off, and the queue said nothing was awaiting review. Both callers now carry an `intent` (`?intent=new`, `?intent=change&for=<id>`) and the console answers it — off reviewer mode it explains that there is no editor because the agent drafts and a person approves; on reviewer mode it opens the Ask composer seeded with that draft request. Second half of the same defect: **the "Turn on reviewer mode" control did nothing.** It was a `<Link>` to `/govern?reviewer=1` clicked from `/govern`, and a query-only push preserves client state, so `useReviewerMode`'s mount-only effect never re-read the param — the mode only appeared after a reload. Both stubbed switches now sit on one subscribable store (`lib/mode.ts`: a seeded query param, a setter, a window event every consumer listens to) and the control is a button, so the whole shell flips in place. `usePermissions` had the identical stale read — latent, since nothing clicks `?access=` today — and moved to the same store. | `app/govern/page.tsx`, `lib/mode.ts`, `lib/reviewer.ts`, `lib/permissions.ts`, `ConceptSidebar.tsx`, `ConceptToolbar.tsx` | T4, T7 |
